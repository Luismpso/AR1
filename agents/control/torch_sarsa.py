...
class TorchSarsaControl(ControlAgent[StateT, ActionT]):
    ...
    def update_transition(self, transition: Transition[StateT, ActionT]) -> None:
        ...
        if self.use_optimizer:
            # 1. Get feature tensor
            phi = self._to_tensor(transition.state, transition.action)
            
            # 2. Clear previous gradients
            self.optimizer.zero_grad()
            
            # 3. Forward pass (predict current Q-value)
            pred = self.model(phi)
            
            # 4. Create target tensor (already detached since computed before the if)
            target_tensor = torch.tensor([target], dtype=torch.float32)
            
            # 5. Compute loss (0.5 * MSE)
            loss = 0.5 * F.mse_loss(pred, target_tensor)
            
            # 6. Backward pass (compute gradients)
            loss.backward()
            
            # 7. Update weights via SGD
            self.optimizer.step()
            
            # 8. Compute delta and append to list
